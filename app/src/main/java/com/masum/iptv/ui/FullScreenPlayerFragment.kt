...
import com.masum.iptv.databinding.FragmentFullScreenPlayerBinding
...
class FullScreenPlayerFragment : Fragment() {
    lateinit var binding:FragmentFullScreenPlayerBinding
    ...
    private val navArgs by navArgs<FullScreenPlayerFragmentArgs>()
    ...
    override fun onCreateView(
        ...
        binding=FragmentFullScreenPlayerBinding.inflate(inflater)
    ...
    private fun initPlayerView() {
        ...
        setButtonEnabled(requireActivity(), buttonAspectRatio, true)


        if (isPiPSupported(requireActivity())) {
            ...
            setButtonEnabled(requireActivity(), buttonPiP!!, false)
        }
        ...
        if (!url.contains("|") && !url.contains("m3u8")) getWebLink(url)
    ...
    private fun initializePlayer(passed: String? = null, headers: Map<String, String>? = null) {
        if(com.google.android.exoplayer2.BuildConfig.DEBUG) Log.i("123321", " passed: " + passed)
        var url=passed?:""
        ...
        (navArgs.origin.ifEmpty { originValue })?.let {
            header.put("Origin",
                it
            )
        }
        (navArgs.referer.ifEmpty { refererValue })?.let {
            header.put("Referer",
                it
            )
        }
        (navArgs.cookie.ifEmpty { cookieValue })?.let {
            header.put("cookie",
                it
            )
        }
        ...
        trackSelector = activity.let { DefaultTrackSelector() }
...
// matching streaming link
        ...
        val defaultHttpDataSourceFactory = DefaultHttpDataSource.Factory()
            ...
            .setUserAgent(userAgentValue?:navArgs.userAgent)
...
//
    ...
    private fun playVideos() {
        ...
        if (buttonPiP != null) setButtonEnabled(requireActivity(), buttonPiP!!, true)
